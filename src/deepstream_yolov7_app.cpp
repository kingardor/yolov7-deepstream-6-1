#include "deepstream_yolov7_app.hpp"
...
namespace YOLOv7 {
  ...
  Odin::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        YOLOv7::Odin::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", YOLOv7::Odin::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!YOLOv7::Odin::set_tracker_properties(nvtracker)) {
  ...
  void Odin::setPaths(guint batch_size) {
    ...
    strdup("models/YOLOv7/config_infer_primary_yolov7.txt");
